...
def test_sed_model_evaluate_sed():
    ...
    static_source = ConstantSED(brightness=brightness)
...
def test_sed_model_evaluate_redshift():
    ...
    times = np.array([0.0, 1.0, 2.0, 3.0, 4.0])
    waves = np.array([4000.0, 5000.0])
    static_source = ConstantSED(brightness=10.0, redshift=0.5, t0=0.0)
...
def test_sed_model_evaluate_bandflux(passbands_dir):
    ...
    static_source = ConstantSED(brightness=f_nu)
    ...
    static_source2 = ConstantSED(brightness=GivenValueList(brightness_list))
