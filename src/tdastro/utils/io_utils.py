...
def _read_lclib_data_from_open_file(input_file):
    ...
    for l_num, line in enumerate(input_file):
        ...
        if key == "COMMENT":
            ...
            curr_id = current_model.get("event_id", "")
            if curr_id != value:
            ...
            for col_idx, col in enumerate(colnames):
                current_model[col].append(col_vals[col_idx])
        else:
...
def _read_snana_output_lc_from_text_file(file_ptr):
    """Read a SNANA output lightcurve from a file.
...
def read_snana_output_lc(file_path):
    ...
    if ".fits" in file_path.suffixes:
        ...
        # Open as a gzipped text file.
        with gzip.open(input_file, "rt") as file_ptr:
            lc, spec_tables = _read_snana_output_lc_from_text_file(file_ptr)
    else:
        # Try to open the file as a regular text file.
        with open(input_file, "r") as file_ptr:
            lc, spec_tables = _read_snana_output_lc_from_text_file(file_ptr)
