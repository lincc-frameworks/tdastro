...
class Passband:
    ...
    def _download_transmission_table(self) -> None:
    ...
    def fluxes_to_bandflux(self, flux_density_matrix, wavelengths_angstrom) -> np.ndarray:
        ...
        # If needed, this is where we would handle interpolation
        # Note that np.searchsorted will automatically handle cases where the flux density grid is coarser
        # than the transmission table grid (though not with interpolation!)
        ...
        # Get only the flux densities that are within the passband
